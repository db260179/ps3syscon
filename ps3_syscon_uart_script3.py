...
class PS3UART(object):
    ...
    def command(self, com, wait = 1, verbose = False):
        ...
        time.sleep(wait)
        answer = self.receive().bytes.decode('ascii').strip()
        if(verbose):
            print(('Answer: ' + answer))
            ...
            if(auth1r[0] == 0 and auth1r[1] != []):
                ...
                if(auth1r[0:0x10] == self.auth1r_header):
                    ...
                    if(data[0x8:0x10] == self.zero[0x0:0x8] and data[0x10:0x20] == self.value and data[0x20:0x30] == self.zero):
                        ...
                        auth2r = self.command('AUTH2 ' + ''.join([format(c, "02X") for c in self.auth2_header + auth2_body]))
                        if(auth2r[0] == 0):
                ...
                if(len(auth1r) == 128):
                    ...
                    if(auth1r[0:0x10] == self.auth1r_header):
                        ...
                        if(data[0x8:0x10] == self.zero[0x0:0x8] and data[0x10:0x20] == self.value and data[0x20:0x30] == self.zero):
                            new_data = data[0x8:0x10] + data[0x0:0x8] + self.zero + self.zero
                            auth2_body = self.aes_encrypt_cbc(self.tb2sc, self.zero, new_data)
                            auth2r = self.command(''.join([format(c, "02X") for c in self.auth2_header + auth2_body]))
...
def main(argc, argv):
    ...
    while True:
        ...
        if(argv[2] == 'CXR'):
            ...
            print(ret[1][0].bytes.decode('ascii'))
