...
#read config
command_file = open ("config.txt", "r")
file_opt = command_file.read()
command_file.close()

file_opt_list = file_opt.split()


sys.argv = []
sys.argv.append("-m")
sys.argv.append(file_opt_list[0])
sys.argv.append(file_opt_list[1])
...
try:
        ...
        while True:
            ...
            if in_data == "none":
                ...
                if(argv[2] == 'CXR'):
                    ...
                    write_code = open("current_code.txt", "w")
                    write_code.write(str('{:08X}'.format(ret[0]) + ' ' +  ' '.join(ret[1])))
                    write_code.close()
                elif(argv[2] == 'SW'):
                    if(len(ret[1]) > 0 and '\n' not in ret[1][0]):
                        print('{:08X}'.format(ret[0]) + ' ' + ' '.join(ret[1]))
    ...
    os.system("msg * Failed to communicate to COM")
